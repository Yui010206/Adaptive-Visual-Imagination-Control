...
class OneStagePromptManager(object):
    ...
    def make_r2r_policy(self, instruction):
        """
        Minimal WM-gated planner with fixed low-level action space.
        Outputs a JSON LIST of plans.
        """

        task_description = (
            "You are an embodied navigation policy.\n"
            "Given a navigation instruction and the current observations (images), your goal is to decide "
            "whether additional perception via a World Model (WM) imagination is needed to enrich the observation for the next decision (chose which observed direction to go as an agent).\n\n"
            "The policy is NOT responsible for long-horizon planning. "
            "Its sole purpose is to determine whether any observation requires WM imagination "
            "to enrich perception for the next decision.\n\n"
            "If WM is needed, select the relevant observation(s) and generate a short probing action plan. "
            "If WM is not needed accross all observations, return an empty list.\n\n"
            "WM is expensive. Only request WM if it is likely to change the next action choice.\n\n"
            "=== Action Space (FIXED) ===\n"
            "- F : move forward 0.25 meters\n"
            "- L : turn left 9 degrees\n"
            "- R : turn right 9 degrees\n"
            "Notes:\n"
            "- To turn 18 degrees or move 0.5 meters, use two consecutive L/R or F actions.\n"
            "- All movement must be composed ONLY from {F, L, R}.\n\n"
            "You MUST output ONLY a JSON array (no explanation text).\n"
        )

        task_description += (
            "\nEach JSON element MUST follow this schema:\n"
            "{\n"
            '  "img_ids": string,          // observation/image IDs (0,1,2,...) requiring WM\n'
            '  "actions": [string1, string2, ...]          // 1-3 actions, each in {"F","L","R"}\n'
            '  "reason": string,          // a short sentence for explaining why WM is needed\n' 
            "}\n\n"
            "Rules:\n"
            "- Do NOT invent observation IDs.\n"
            "- Action sequences should be short and purposeful.\n"
            "- Prefer shorter actions if call world models.\n"
        )
        ...
        prompt = (
            ...
            # "Generate the JSON plan list now."
        )
