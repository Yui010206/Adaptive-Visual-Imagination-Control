...
class Model:
    def __init__(self):
        ...
        self.model = SGMWrapper(load_model(device="cpu", verbose=True).eval()).to(device)
...
def svc_main(
    ...
    **overwrite_options,
    ...
    MODEL, AE, CONDITIONER, DENOISER = model.model, model.ae, model.conditioner, model.denoiser
    ...
    options["seed"] = 23
    options["replace_or_include_input"] = True
    ...
    if data_items is not None:
        ...
        # scenes = scenes = glob.glob(osp.join(data_path, "*"))
        # scenes = glob.glob(osp.join(data_path, "*.jpg")) + glob.glob(osp.join(data_path, "*.png"))
        scenes = [osp.join(osp.dirname(osp.dirname(data_path)),"step_0","img_0.png")]
    ...
    for scene in scenes:
        ...
        # img_paths = sorted(glob.glob(osp.join(save_path_scene, "*.png")))
