...
class ChatAPI:
    def __init__(
        ...
        self.usage_history = []   # list of dicts
    ...
    def get_system_response_with_content(self, sys_prompt, content) -> str:
        try:
            ...
            usage_tokens = response.usage.total_tokens
            cost = usage_tokens * self.price / 1000

            # ---- persist usage ----
            self.total_usage_tokens += usage_tokens
            self.cost += cost

            self.usage_history.append({
                "model": self.model,
                "usage_tokens": usage_tokens,
                "price_per_1k": self.price,
                "cost": cost,
            })

            print(
                f"[ChatGPT] model={self.model}, usage_tokens={usage_tokens}, "
                f"total_tokens={self.total_usage_tokens}, total_cost=${self.cost:.5f}"
            )

            if usage_tokens > self.max_limit:
                print("[ChatGPT] truncate the conversation to avoid token usage limit")
                self.truncate()
